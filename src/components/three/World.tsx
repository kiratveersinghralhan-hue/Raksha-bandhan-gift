...
function GiftScene3D({ motion, quality, reducedMotion }: { motion: MutableRefObject<GiftMotionState>; quality: QualityLevel; reducedMotion: boolean }) {
  return <LuxuryGiftBox motion={motion} quality={quality} reducedMotion={reducedMotion} />;
}
...
function MemoryFrame({ index, active, interactive, onSelect, reducedMotion }: { index: number; active: number | null; interactive: boolean; onSelect: (index: number) => void; reducedMotion: boolean }) {
  ...
  const imageHeight = memory.aspect < 1 ? 1.22 : 0.9;
...
function GlobeScene3D({ quality, reducedMotion, interactive }: { quality: QualityLevel; reducedMotion: boolean; interactive: boolean }) {
  ...
  return (
    <>
      ...
      <group ref={globe} position={compact ? [0, 0.1, -0.25] : [-0.82, -0.02, -0.26]} scale={compact ? 0.96 : 0.93} rotation={[0.08, -0.48, -0.03]} onPointerDown={interactive ? down : undefined} onPointerMove={interactive ? move : undefined} onPointerUp={interactive ? up : undefined} onPointerLeave={interactive ? up : undefined}>
        <mesh>
...
function EnvelopeScene3D({ selectedMessage, interactive, reducedMotion, onSelect }: { selectedMessage: string | null; interactive: boolean; reducedMotion: boolean; onSelect: (id: string) => void }) {
  ...
  return (
    <group scale={compact ? 0.63 : 1}>
      {messages.map((message, index) => <Envelope key={message.id} index={index} position={positions[index]} selected={selectedMessage === message.id} interactive={interactive && selectedMessage === null} onSelect={() => onSelect(message.id)} reducedMotion={reducedMotion} />)}
    </group>
  );
}
